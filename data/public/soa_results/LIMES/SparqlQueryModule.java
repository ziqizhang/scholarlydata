                            ...
                            for (int i = 0; i < kb.getProperties().size(); i++) {
                                ...
                                if (soln.contains("v" + i)) {
                                    ...
                                    for (String propertyDub : kb.getFunctions().get(propertyLabel).keySet()) {
                                        ...
                                        /*CODE ADDED TO WORKAROUND SLOW GRAPH FLATTERNING. ONLY WORKS FOR scholarlydata project*/
                                        if(value.contains("|")){
                                            String[] values = value.split("\\|");
                                            for(String v: values)
                                                cache.addTriple(uri, propertyDub, v);
                                        }
                                        /*CODE ADD complete*/
                                        //original code: cache.addTriple(uri, propertyDub, value);
                                    }
                                }
                            }
                        } catch (Exception e) {
